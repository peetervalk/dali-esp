...
| Protocol separation | **Directionally strong.** The reusable C stack is independent of ESPHome, but protocol sequences, event interpretation, and installation-specific sensor policy still need to move out of `dali_component.cpp`. |
...
- The tracked worktree was clean before this status/YAML update.
...
- The command console includes `raw`, `raw2`, memory, DTR, instance-query, and
  ...
  hardware validation.
- ESPHome exposes discovery, not a guarded commissioning workflow. Public
  documentation must not claim HA commissioning until it exists.
...
The two `_local` files are deliberately ignored by Git. Inspect and back them up
separately; normal `git status` does not show their changes.
...
Polling is currently authoritative for occupancy. Unsolicited Steinel traffic is
useful telemetry, but must not be treated as a proven generic one-byte sensor value
until Part 103 event decoding and device-profile mapping are corrected.

## Prioritized Work

### P0 — Protocol correctness and conformance

- Correct Part 103 command/event discrimination, source-scheme and instance
  decoding, full event-information width, and DT301 event constants. Test with
  independent real-frame vectors rather than values copied from the implementation.
...
- Replace the generic one-byte DT303 whitelist with typed event/device-profile
  handling. Prefer “event triggers an immediate poll” where an event is not itself
  a sensor value.
...
- Correct DALI-2 dispatch and Find Couplers handling so encoded source scheme,
  instance/type selector, and event information are retained and matched once.
- Move raw opcodes, event decoding, memory/DTR sequences, and site-specific sensor
  rules out of `dali_component.cpp` into reusable typed C APIs.
...
- Pin release examples in the README to a release tag and keep any `main` example
  explicitly labeled for development.
